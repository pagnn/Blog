...
Base.prototype.next=function(){
	for (var i = 0; i < this.elements.length; i ++) {
		...
		if(this.elements[i]==null) throw new Error('找不到当前节点的下一个同级节点');
...
Base.prototype.prev=function(){
	for (var i = 0; i < this.elements.length; i ++) {
		...
		if(this.elements[i]==null) throw new Error('找不到当前节点的上一个同级节点');
...
Base.prototype.css = function (attr, value) {
	for (var i = 0; i < this.elements.length; i ++) {
		if (arguments.length == 1) {
			return parseInt(getStyle(this.elements[i], attr))+"px";
		}
...
Base.prototype.animate = function (obj) {
	for (var i = 0; i < this.elements.length; i ++) {
		...
		if (alter != undefined && target == undefined) {
			...
			throw new Error('alter增量或target,mul目标量必须传一个！');
		}
		...
		element.timer = setInterval(function () {
			/*问题1:多个动画执行了多个列队动画，我们要求不管多少个动画只执行一个列队动画
			  ...
			  解决2：多个同步动画按最后一个同步动画执行完毕后在清理掉就可以了	
			*/
			var flag=true;
			for(var i in mul){
